...
#include "definition.h"
  ...
  use sim_data , only: sim_riemann

contains

subroutine soln_PPMtracing(dt, V, vL, vR, lambda, leig0, reig0)
  ...
  integer :: kWaveNum, i, j, var
  real :: lambdaDtDx, delC1, delC2
  real, dimension(NSYS_VAR) :: sigL, sigR, vecL, vecR, C0, C1, C2
  ...
  do var = DENS_VAR, PRES_VAR
     C0(var) = V(var)
     C1(var) = (vR(var)-vL(var))
     C2(var) = 2.*(vR(var) + vL(var) - 2.*V(var))
  end do
...
!!$     C0(DENS_VAR:PRES_VAR) = V(DENS_VAR:PRES_VAR) - C2(DENS_VAR:PRES_VAR)/12.
...
!!$  vL(DENS_VAR:PRES_VAR) = C0(DENS_VAR:PRES_VAR) - C1(:)*gr_dx/2. + C2(:)*gr_dx*gr_dx/4.
!!$  vR(DENS_VAR:PRES_VAR) = C0(DENS_VAR:PRES_VAR) + C1(:)*gr_dx/2. + C2(:)*gr_dx*gr_dx/4.
