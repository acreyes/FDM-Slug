...
#include "definition.h"
  ...
  procedure (gp_kernel1D), pointer :: gp_K
  ...
  if (gp_kernel == 'SE') then
     gp_K => SE
  else
     !default to SE kernel
     gp_K => SE
  end if
  ...
  do i = 1,N
     do j = 1,N
        C(i,j) = gp_K(stencil(i),stencil(j),Xdel)
     end do
     T(1, i) = gp_K(-0.5, stencil(i), Xdel)
     T(2, i) = gp_K( 0.5, stencil(i), Xdel)
  end do
...
!!$
!!$  !try out the analytic weights generated by mathematica
!!$  Z(1,:) = analytic_weights3(C, T(1,:))
!!$  Z(2,:) = analytic_weights3(C, T(2,:))
  ...
  do m = 1, N
     ...
     do i = 1, N
        do j = 1, N
           Ck(i,j) = gp_K(stencil(i),stencil(j),Xdel)
        end do
        Tk(1,i) = gp_K(-0.5, stencil(i), Xdel)
        Tk(2,i) = gp_K( 0.5, stencil(i), Xdel)
     end do
     call chol(Ck,N,Lk)
     call solve_CZT(Ck, Zk(:,:,m), Tk, Lk, N)
!!$     Zk(1,:,m) = analytic_weights2(Ck, Tk(1,:))
!!$     Zk(2,:,m) = analytic_weights2(Ck, Tk(2,:))
