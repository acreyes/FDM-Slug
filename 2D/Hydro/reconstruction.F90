...
contains
  ...
  subroutine recon1D_ij(ibeg, iend, sm, sp, dir, V, recon)
    ...
    real, dimension(gp_radius+1) :: smth
    Nx = sp-sm+1
...
!!$          !check if there is a physica boundary on the stencil
          ...
          do var = 1, NUMB_VAR
             ...
             if (var < NSYS_VAR+1) then
                call gp_betas(stencil(:,var), gp_radius, smth)
                gr_betas(var,i,j,dir) = smth(gp_radius)
             end if
          end do       !var
